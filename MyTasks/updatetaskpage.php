...
$task = query("SELECT * FROM tasks WHERE id_tasks = $id_tasks")[0];
...
if ( isset($_POST["submit"])) {
	if ( updatetask($_POST) > 0 ) {
		...
		document.location.href = 'updatetaskpage.php';
		</script>";
	}
		...
		<div class="content cf container">
			<div class="row justify-content-center">
					...
					<nav class="nav flex-column nav-tabs">
						<li class="nav-item dropdown">
							...
							<div class="dropdown-menu">
								<a class="dropdown-item" href="#"><i class="fas fa-plus-square"></i>&nbsp;Add Task</a>
							</div>
						</li>
					</nav>
				</div>
				...
				<div class="mainwrapper col-md-9">
					<div class="judulmain">
						<div class="row">
							<div class="col-8"><h4><i class="fas fa-tasks"></i></i>&nbsp;Edit Tasks</h4></div>
						</div>					
					</div>
					<form class="formedittask" action="" method="post">
						<input type="hidden" name="id_tasks" value="<?= $task["id_tasks"]; ?>">
						<div class="form-group">
							<label for="title">Title</label>
							<input type="text" class="form-control" id="title" name="title" required="" value="<?= $task["title"]; ?>" autofocus autocomplete="off">
						</div>
						<div class="form-group">
							<label for="description">Description</label>
							<input class="form-control" id="description" name="description" required="" value="<?= $task["description"]; ?>"></input>
						</div>
						<div class="form-group">
							<label for="status">Status</label>
							<input type="text" class="form-control" id="title" name="status" required="" value="<?= $task["status"]; ?>">
							<small id="" class="form-text">
								<i>You can can type status like : Queued or Completed</i>
							</small>
						</div>
						<label>Last Modified : </label>
						<label><strong><?= $task["updated"]; ?></strong></label>
						<br>
